...
* Реализовать новый класс для ведения статистики в пакете `models`. Наследовать этот класс от родительского `Stats`.
* Определить новый элемент перечисления в `DataType` пакета `utils`.
* Сохранить новый тип данных в мапу в методе `initializeStatsMap` класса `Main`.
* В классе `DataTypeDeterminer` добавить методы для идентификации нового типа данных.
* В методе `updateDataTypeStats` класса `FileService` в конструкции `switch-case` добавить вызов метода обновления статистики для добавляемого типа данных.
* Вывод статистики по новому классу будет произведен автоматически, так как в методе `for-each` мы перебираем все сохраненные ключи в мапе статистики.
